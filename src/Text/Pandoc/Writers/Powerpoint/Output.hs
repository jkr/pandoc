...
speakerNotesSlideNumber :: Int -> Element
speakerNotesSlideNumber pgNum =
  mknode "p:sp" [] $
  [ mknode "p:nvSpPr" [] $
    [ mknode "p:cNvPr" [ ("id", "4")
  ...
  , mknode "p:txBody" [] $
    ...
    , mknode "a:lstStyle" [] ()
    , mknode "a:p" [] $
      -- TODO: this should be pulled out from the notesMaster
      -- file. Since we're always using the same notesMaster file,
      -- this will work for now, but it will be more robuse to extract
      -- it ourselves.
      [ mknode "a:fld" [ ("id", "{18BDFEC3-8487-43E8-A154-7C12CBC1FFF2}")
                       , ("type", "slidenum")
                       ]
