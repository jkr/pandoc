...
findUsableIndex :: [Ref] -> Int -> Int
findUsableIndex refs i = do
  let rfLbls = map (\(r,_,_) -> r) refs
  if (text (show i)) `elem` rfLbls
    then findUsableIndex refs (i + 1)
    else i
...
getNextIndex = do
  ...
  refs <- gets stRefs
  i <- (+ 1) <$> gets stLastIdx
  return $ findUsableIndex (prevRefs ++ refs) i
